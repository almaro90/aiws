...
CREATE TABLE spec_revisions (
    ...
    created_at TEXT NOT NULL,
    UNIQUE(task_id, cycle_id, revision)
) STRICT;
...
INSERT INTO task_cycles(id, task_id, number, created_at, completed_at)
SELECT 'cyc_' || substr(id, 5), id, 1, created_at,
       CASE WHEN status = 'done' THEN updated_at ELSE NULL END
FROM tasks;

INSERT INTO task_messages(id, task_id, cycle_id, type, text, created_at)
SELECT 'msg_' || substr(id, 5), id, 'cyc_' || substr(id, 5), 'initial_request', user_request, created_at
FROM tasks;

INSERT INTO spec_revisions(id, task_id, cycle_id, revision, content, created_at)
SELECT 'spc_' || substr(id, 5), id, 'cyc_' || substr(id, 5), 1, curator_spec, updated_at
FROM tasks WHERE trim(curator_spec) <> '';
...
INSERT INTO question_answers(id, question_id, cycle_id, revision, answer_text, selected_option_ids_json, created_at)
SELECT 'ans_' || substr(id, 5), id, cycle_id, 1, answer_text, selected_option_ids_json, answered_at
FROM questions WHERE answered_at IS NOT NULL;

INSERT INTO deliveries(id, task_id, branch_name, pr_url, created_at, updated_at)
SELECT 'dlv_' || substr(t.id, 5), t.id,
       (SELECT r.branch_name FROM runs r WHERE r.task_id = t.id AND r.branch_name IS NOT NULL ORDER BY r.created_at DESC, r.id DESC LIMIT 1),
       coalesce(t.pr_url, (SELECT r.pr_url FROM runs r WHERE r.task_id = t.id AND r.pr_url IS NOT NULL ORDER BY r.created_at DESC, r.id DESC LIMIT 1)),
       t.created_at, t.updated_at
FROM tasks t
WHERE t.pr_url IS NOT NULL OR EXISTS (SELECT 1 FROM runs r WHERE r.task_id = t.id AND r.branch_name IS NOT NULL);

UPDATE tasks SET current_delivery_id = 'dlv_' || substr(id, 5)
WHERE EXISTS (SELECT 1 FROM deliveries d WHERE d.id = 'dlv_' || substr(tasks.id, 5));
UPDATE task_cycles SET delivery_id = (SELECT current_delivery_id FROM tasks WHERE tasks.id = task_cycles.task_id);
UPDATE runs SET delivery_id = (SELECT current_delivery_id FROM tasks WHERE tasks.id = runs.task_id)
