# Git gestionado, curation y automatización Codex — v0.3

## Flujo

1. El administrador instala la GitHub App desde **Automation**.
2. AIWS valida el callback firmado, consulta la instalación y guarda solo su identidad.
...
8. Codex delega el sandbox en el contenedor agente ya restringido (`--dangerously-bypass-approvals-and-sandbox` evita un segundo sandbox `bwrap`). Después, el manager hace commit, push y crea un draft PR con un token de instalación efímero.
...
- `server`: posee SQLite, logs, clave privada de GitHub App y emite tokens de instalación de corta vida solo al actor `system`.
...
- Los tokens Git se entregan mediante `GIT_ASKPASS`; no aparecen en clone URLs ni argumentos persistidos.
- Los logs se redactan, limitan a 5 MB, se guardan bajo `/data/run-logs` y se sirven solo por endpoint autenticado.
- La callback GitHub es pública porque las cookies `SameSite=Strict` no cruzan el redirect; solo acepta un `state` HMAC con expiración emitido desde una sesión autenticada.
...
docker run --rm --user root \
  ...
  aiws-agent:0.5.1 chown -R 1000:1000 /codex-home

docker run --rm -it \
  -v aiws-codex-auth:/codex-home \
  -e CODEX_HOME=/codex-home \
  aiws-agent:0.5.1 codex login --device-auth
```
...
- GitHub.com únicamente; no GitHub Enterprise Server ni GitLab.
