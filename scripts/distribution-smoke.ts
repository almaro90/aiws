...
try {
  ...
  await Promise.all([
    run(["docker", "build", "--target", "server", "-t", `${namespace}/aiws:0.5.1`, "."]),
    run([
      "docker",
      "build",
      "--target",
      "runner-manager",
      "-t",
      `${namespace}/aiws-runner-manager:0.5.1`,
      ".",
    ]),
    run(["docker", "build", "--target", "agent", "-t", `${namespace}/aiws-agent:0.5.1`, "."]),
  ]);
  ...
  for (const image of ["aiws", "aiws-runner-manager", "aiws-agent"]) {
    await runResult(["docker", "image", "rm", "--force", `${namespace}/${image}:0.5.1`]);
  }
