...
try {
  ...
  await Promise.all([
    run(["docker", "build", "--target", "server", "-t", `${namespace}/aiws:0.8.0`, "."]),
    run([
      "docker",
      "build",
      "--target",
      "runner-manager",
      "-t",
      `${namespace}/aiws-runner-manager:0.8.0`,
      ".",
    ]),
    run(["docker", "build", "--target", "agent", "-t", `${namespace}/aiws-agent:0.8.0`, "."]),
  ]);
  ...
  for (const image of ["aiws", "aiws-runner-manager", "aiws-agent"]) {
    await runResult(["docker", "image", "rm", "--force", `${namespace}/${image}:0.8.0`]);
  }
