---
name: aiws-workflow
description: Opera Tasks de AIWS mediante su CLI HTTP y JSON. Usar para preparar y enviar Drafts, curar Tasks Curating, revisar agregados y adjuntos, crear o responder Questions, preparar Curator Specs, inspeccionar Runs por tipo, reclamar implementación, registrar una PR, completar tareas o consultar Activity respetando versiones y estados.
---
...
1. Mostrar la Task Draft y ajustar título o petición si hace falta:
...
2. Añadir los Attachments necesarios.
3. Enviar explícitamente la Task; esta operación congela `userRequest`:
...
4. En un Project gestionado, comprobar Runs con `aiws --json run list --task TASK_ID --kind curation`. No intentar curarlo en paralelo.
...
2. Mostrar el agregado y reclamarlo antes de modificar el repositorio:
...
3. Continuar solo si el claim devuelve éxito. Si devuelve conflicto, elegir otra Task o recargar; otro agente pudo ganarlo.
4. Trabajar en `project.repositoryPath` siguiendo la Curator Spec y las instrucciones propias de ese repositorio.
5. Si aparece una ambigüedad bloqueante, crear una Question con la versión actual. No inventar la respuesta ni completar la Task.
6. Si se creó una PR externamente, registrar o reemplazar su URL:
...
7. Ejecutar las verificaciones exigidas por el repositorio. Done no requiere PR.
8. Completar únicamente desde Implementing:
