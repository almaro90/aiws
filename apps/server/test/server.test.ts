...
describe("Hito 3 server", () => {
  ...
  test("exposes health publicly and protects every other API route", async () => {
    ...
    try {
      ...
      expect(await health.json()).toEqual({ status: "ok", version: "0.5.1" });
  ...
  test("reports unhealthy when the SQLite health probe fails", async () => {
    ...
    try {
      ...
      expect(response.status).toBe(503);
      expect(await response.json()).toEqual({ status: "unhealthy", version: "0.5.1" });
    } finally {
      await fixture.close();
    }
  });
  ...
  test("serves the authenticated OpenAPI document", async () => {
    ...
    try {
      ...
      expect(await response.json()).toEqual({ openapi: "3.1.0", info: { version: "0.5.1" } });
    } finally {
      await fixture.close();
    }
  });
...
async function createFixture(
  ...
  const app = createApp({
    ...
    messages,
    repositoryValidator,
    openApiDocument: { openapi: "3.1.0", info: { version: "0.5.1" } },
